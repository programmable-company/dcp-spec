...
- Adds requirements specific to Estonian context (e.g. mandating that a Programmable Company corresponding to a registered Estonian legal person carries an `ee.registrikood` identifier).
...
- How does X-Road's authorization model map to Programmable Company's capability model? Probably as a capability (`ee.x-road/v1`) declaring the X-Road subsystem identifier. TODO(SEP-XXXX).
