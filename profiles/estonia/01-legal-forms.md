...
- **Form changes.** Estonian law permits some inter-form transitions (e.g. `OÜ` → `AS`). Does Programmable Company model this as a `core.lifecycle.legalForm-changed` event, or as a dissolution-and-reformation? The full profile will need to take a position. TODO(SEP-XXXX).
