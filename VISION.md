...
## The Agentic State needs a Programmable Company counterpart
...
Programmable Company exists to remove this bottleneck by giving the company itself a machine-readable, cryptographically-verifiable representation — one that is portable across systems, grounded in the same identity primitives the state already uses, and legible to agentic systems without bespoke integration.

## What a programmable company is

A programmable company, in the sense this specification uses the term, is a company whose governance, authority relationships, and state exist as signed artifacts rather than as the backend of a particular product.
...
In most jurisdictions, a specification of this kind would require inventing primitives that do not yet exist — a qualified signature stack, a registry API, a notion of machine-verifiable mandate. In Estonia, those primitives already exist and are legally binding. The specification's job is to compose them, not to bootstrap them.
...
## What the specification aims to standardize
...
Programmable Company is not a product. It will not be sold, licensed, or gated. There is no hosted offering that "is" Programmable Company.

Programmable Company is not a runtime. It does not prescribe how an implementation should be built, deployed, or operated. Conforming implementations may be libraries, services, SaaS products, or bespoke in-house systems.

Programmable Company is not a UI framework. It says nothing about how company data is displayed to humans.

Programmable Company is not a replacement for legal systems. It operates within existing company law and existing signature law; it does not attempt to codify either, and where the specification and applicable law differ, applicable law governs.

Programmable Company is not an attempt to define what a company should be. It is a convention for representing companies as they already are, in a form that agentic systems can work with.
...
Substantive community contributions are not yet invited. The specification needs an initial coherent shape before collaborative drafting is productive. That shape will emerge from a small number of drafts over the coming months, at which point the contribution model will be defined and opened.
