...
| `cty` | `application/pc+json` for Programmable Company JSON artifacts. |
...
The `kid` claim is a stable identifier for the signing key. Programmable Company itself imposes no internal structure on the `kid` string beyond uniqueness within a producer's signing infrastructure.
...
A valid Programmable Company signature establishes:
...
A document may need multiple signatures — a board resolution signed by every board member, a contract signed by both parties. Programmable Company expresses these as **separate sidecar files**, one per signer:
